...
# === MODEL DEFINITIONS ===
def fcn(x, p):
    # GaussianPb_Expb model optimized
...
def FitSimultaneousA2B(PL, bminForFit, etamin, etamax, base_path="/Users/hariprashadravikumar/Lattice_QCD_TMD_PhD/sivers_TMD_PhD_project/save_h5_A12B_A2B/"):
    print(f"\n--- Simultaneous Fit (GaussianPb_Expb) : PL={PL}, b_min>={bminForFit}, eta in [{etamin}, {etamax}] ---")
    ...
    current_prior = {
        'f'    : gv.gvar(0.487, 0.2),
        'k1_re': gv.gvar(11.96, 2.0),
        'k2_re': gv.gvar(-34.3, 4.0),
        'a_re' : gv.gvar(5.94, 1.0),
        'c_re' : gv.gvar(0.114, 0.05),
        'd_re' : gv.gvar(0.588, 0.2),
    ...
    x_dict = {
        'eta_re': kin_re[:, 0], 'bL_PL_re': kin_re[:, 1] * PL, 'bL_PL_sq_re': (kin_re[:, 1] * PL)**2, 'b_mag_re': np.sqrt(kin_re[:, 1]**2 + kin_re[:, 2]**2),
        'eta_im': kin_im[:, 0], 'bL_PL_im': kin_im[:, 1] * PL, 'bL_PL_sq_im': (kin_im[:, 1] * PL)**2, 'b_mag_im': np.sqrt(kin_im[:, 1]**2 + kin_im[:, 2]**2),
        'eta_reA12B': kin_reA12B[:, 0], 'bL_PL_reA12B': kin_reA12B[:, 1] * PL, 'bL_PL_sq_reA12B': (kin_reA12B[:, 1] * PL)**2, 'b_mag_reA12B': np.sqrt(kin_reA12B[:, 1]**2 + kin_reA12B[:, 2]**2)
    }
