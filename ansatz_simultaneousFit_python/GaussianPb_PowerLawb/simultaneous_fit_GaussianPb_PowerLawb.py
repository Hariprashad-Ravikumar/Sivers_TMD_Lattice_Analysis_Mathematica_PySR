...
# === MODEL DEFINITIONS ===
def fcn(x, p):
    # GaussianPb_PowerLawb model optimized
    f_eta_re = p['f'] * x['eta_re']
    c_term_re = p['c_re'] * (1 - p['k1_re']/x['eta_re'] - p['k2_re']/x['eta_re']**2) * x['bL_PL_sq_re']
    denom_re = (1 + p['d_re'] * x['b_mag_sq_re']) ** p['j_re']
    val_re = p['a_re'] * gv.exp(-(f_eta_re + c_term_re)) / denom_re
    
    f_eta_im = p['f'] * x['eta_im']
    c_term_im = p['c_im'] * x['bL_PL_sq_im']
    denom_im = (1 + p['d_im'] * x['b_mag_sq_im']) ** p['j_im']
    val_im = p['a_im'] * x['bL_PL_im'] * gv.exp(-(f_eta_im + c_term_im)) / denom_im
    
    f_eta_reA12B = p['f'] * x['eta_reA12B']
    c_term_reA12B = p['c_reA12B'] * x['bL_PL_sq_reA12B']
    denom_reA12B = (1 + p['d_reA12B'] * x['b_mag_sq_reA12B']) ** p['j_reA12B']
    val_reA12B = -p['a_reA12B'] * gv.exp(-(f_eta_reA12B + c_term_reA12B)) / denom_reA12B
...
def FitSimultaneousA2B(PL, bminForFit, etamin, etamax, base_path="/Users/hariprashadravikumar/Lattice_QCD_TMD_PhD/sivers_TMD_PhD_project/save_h5_A12B_A2B/"):
    print(f"\n--- Simultaneous Fit (GaussianPb_PowerLawb) : PL={PL}, b_min>={bminForFit}, eta in [{etamin}, {etamax}] ---")
    ...
    current_prior = {
        'f'    : gv.gvar(0.4896, 0.2),
        'k1_re': gv.gvar(12.02, 2.0),
        'k2_re': gv.gvar(-34.8, 4.0),
        'a_re' : gv.gvar(3.68, 1.0),
        'c_re' : gv.gvar(0.110, 0.05),
        'd_re' : gv.gvar(0.084, 0.05),
        'j_re' : gv.gvar(2.16, 0.5),
        'a_im' : gv.gvar(0.26, 0.2),
        'c_im' : gv.gvar(0.3, 0.2),
        'd_im' : gv.gvar(0.2, 0.1),
        'j_im' : gv.gvar(1.0, 0.5),
        'a_reA12B' : gv.gvar(0.6, 0.2),
        'c_reA12B' : gv.gvar(0.02, 0.2),
        'd_reA12B' : gv.gvar(0.02, 0.1),
        'j_reA12B' : gv.gvar(2.0, 0.5),
    }

    # PRECALCULATE KINEMATICS
    x_dict = {
        'eta_re': kin_re[:, 0], 'bL_PL_re': kin_re[:, 1] * PL, 'bL_PL_sq_re': (kin_re[:, 1] * PL)**2, 'b_mag_sq_re': kin_re[:, 1]**2 + kin_re[:, 2]**2,
        'eta_im': kin_im[:, 0], 'bL_PL_im': kin_im[:, 1] * PL, 'bL_PL_sq_im': (kin_im[:, 1] * PL)**2, 'b_mag_sq_im': kin_im[:, 1]**2 + kin_im[:, 2]**2,
        'eta_reA12B': kin_reA12B[:, 0], 'bL_PL_reA12B': kin_reA12B[:, 1] * PL, 'bL_PL_sq_reA12B': (kin_reA12B[:, 1] * PL)**2, 'b_mag_sq_reA12B': kin_reA12B[:, 1]**2 + kin_reA12B[:, 2]**2
    }
...
if __name__ == "__main__":
    ...
    ansatz_name = "GaussianPb_PowerLawb"
