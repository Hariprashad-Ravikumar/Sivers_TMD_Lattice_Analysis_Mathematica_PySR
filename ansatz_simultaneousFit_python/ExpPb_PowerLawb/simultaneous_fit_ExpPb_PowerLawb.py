...
# === MODEL DEFINITIONS ===
def fcn(x, p):
    # ExpPb_PowerLawb model optimized
...
def FitSimultaneousA2B(PL, bminForFit, etamin, etamax, base_path="/Users/hariprashadravikumar/Lattice_QCD_TMD_PhD/sivers_TMD_PhD_project/save_h5_A12B_A2B/"):
    print(f"\n--- Simultaneous Fit (ExpPb_PowerLawb) : PL={PL}, b_min>={bminForFit}, eta in [{etamin}, {etamax}] ---")
    ...
    current_prior = {
        'f'    : gv.gvar(0.473, 0.2),
        'k1_re': gv.gvar(11.79, 2.0),
        'k2_re': gv.gvar(-33.0, 4.0),
        'a_re' : gv.gvar(3.53, 1.0),
        'c_re' : gv.gvar(0.49, 0.2),
        'd_re' : gv.gvar(0.096, 0.05),
        'j_re' : gv.gvar(2.06, 0.5),
    ...
    x_dict = {
        'eta_re': kin_re[:, 0], 'bL_PL_re': kin_re[:, 1] * PL, 'abs_bL_PL_re': np.abs(kin_re[:, 1] * PL), 'b_mag_sq_re': kin_re[:, 1]**2 + kin_re[:, 2]**2,
        'eta_im': kin_im[:, 0], 'bL_PL_im': kin_im[:, 1] * PL, 'abs_bL_PL_im': np.abs(kin_im[:, 1] * PL), 'b_mag_sq_im': kin_im[:, 1]**2 + kin_im[:, 2]**2,
        'eta_reA12B': kin_reA12B[:, 0], 'bL_PL_reA12B': kin_reA12B[:, 1] * PL, 'abs_bL_PL_reA12B': np.abs(kin_reA12B[:, 1] * PL), 'b_mag_sq_reA12B': kin_reA12B[:, 1]**2 + kin_reA12B[:, 2]**2
    }
