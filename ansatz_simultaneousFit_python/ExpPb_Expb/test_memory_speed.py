...
def TestOneSample(PL=-1, bminForFit=3, etamin=6, etamax=10, base_path="/Users/hariprashadravikumar/Lattice_QCD_TMD_PhD/sivers_TMD_PhD_project/save_h5_A12B_A2B/"):
    ...
    y_sample_i = combined_jk[:, 0]  # Just take the first jackknife sample
    
    print("--- Testing memory and speed for 1 optimized Jackknife sample ---")
    t0 = time.time()
    ...
    print(f"Time for gv.gvar setup: {t1 - t0:.4f} seconds")
    print(f"Time for nonlinear_fit: {t3 - t2:.4f} seconds")
    print(f"Total time taken: {(t1 - t0) + (t3 - t2):.4f} seconds")
