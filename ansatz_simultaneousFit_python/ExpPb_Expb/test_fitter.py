...
for fitter in ['scipy_least_squares', 'gsl_multifit']:
    t0 = time.time()
    fit = lsqfit.nonlinear_fit(data=(x_dict, y_dict), prior=current_prior, fcn=fcn, fitter=fitter)
    t1 = time.time()
    print(f"Fitter: {fitter:20} Time: {t1-t0:.4f}s Chi2/dof: {fit.chi2/fit.dof:.4f}")
