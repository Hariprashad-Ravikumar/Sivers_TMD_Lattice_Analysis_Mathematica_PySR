...
for tol in [1e-12, 1e-8, 1e-4, 1e-3, 1e-2]:
    t0 = time.time()
    fit = lsqfit.nonlinear_fit(data=(x_dict, y_dict), prior=current_prior, fcn=fcn, tol=(tol, tol, tol))
    t1 = time.time()
    print(f"Tol: {tol} Time: {t1-t0:.4f}s Chi2/dof: {fit.chi2/fit.dof:.4f}  pmean[f]={fit.pmean['f']:.5f}")

for cut in [1e-8, 1e-6, 1e-4, 1e-3]:
    t0 = time.time()
    fit = lsqfit.nonlinear_fit(data=(x_dict, y_dict), prior=current_prior, fcn=fcn, svdcut=cut)
    t1 = time.time()
    print(f"svdcut: {cut} Time: {t1-t0:.4f}s Chi2/dof: {fit.chi2/fit.dof:.4f}  pmean[f]={fit.pmean['f']:.5f}")
